...
def run_ssh_blocks(test_data, ip, user, password, sheet_name):
    ...
    for block in test_data:
        if isinstance(block, tuple):
            ...
            # --- НОВОЕ: Кастомные учетки для конкретного блока команд ---
            custom_user = block.get("user", user)
            custom_pass = block.get("password", password)
            cmds = block.get("commands", [])
            
            # Открываем новую SSH сессию под новым пользователем
            ssh_sender.send_command(ip, custom_user, custom_pass, "Switch#", cmds, sheet_name, test_index)
            
            # Увеличиваем индекс тестов
